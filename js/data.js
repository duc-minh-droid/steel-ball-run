...
/* ---------------- Consumable items ---------------- */
SBR.ITEMS = {
  canteen:   { name: 'SPW Canteen', glyph: '水', color: '#6aa0c8', price: 18, target: 'ally', desc: 'Heal 14 HP. Speedwagon Foundation issue: they supply anyone fighting the strange.', field: true, use: x => x.heal(x.target, 14) },
  jerky:     { name: 'Kakyoin\'s Cherries', glyph: '桜', color: '#c8323c', price: 22, target: 'allAllies', desc: 'Heal the whole party 7 HP. Enough to go round. Rero rero rero.', field: true, use: x => x.allies.forEach(a => x.heal(a, 7)) },
  coffee:    { name: 'Iggy\'s Coffee Gum', glyph: '珈', color: '#8a5a30', price: 20, target: 'ally', desc: '+1 Energy now and Second Wind for 3 turns. The one thing that dog ever begged for.', use: x => { x.energy(x.target, 1); x.status(x.target, 'secondwind', 0, 3); } },
  bandage:   { name: 'Tonio\'s Mineral Water', glyph: '泉', color: '#9fd0f0', price: 15, target: 'ally', desc: 'Remove all debuffs (except Infinite Rotation) and heal 4. Pearl Jam washes the sickness out, mostly through the eyes.', field: true, use: x => { x.cleanse(x.target, 99); x.heal(x.target, 4); } },
  dynamite:  { name: 'Stroheim\'s Grenade', glyph: '爆', color: '#5a6a4a', price: 30, target: 'allEnemies', desc: 'Deal 12 damage to all enemies. German science, pin already loose.', use: x => x.enemies.forEach(e => x.dmg(e, 12, {}, { noCrit: true, label: 'BOOM' })) },
  salts:     { name: 'Heaven\'s Door Page', glyph: '書', color: '#f6ecd8', price: 35, target: 'allyDead', desc: 'Revive a fallen ally at 30% HP. Rohan has already written "gets back up" on it.', use: x => x.revive(x.target, 0.3) },
  whiskey:   { name: 'Zeppeli\'s Hamon Wine', glyph: '酒', color: '#8a1a3a', price: 16, target: 'ally', desc: 'Empowered 3 turns, but Blinded 1. Hamon runs through wine. So does the wine.', use: x => { x.status(x.target, 'empower', 0, 3); x.status(x.target, 'blind', 0, 1); } },
  snakeoil:  { name: 'Aqua Necklace Bottle', glyph: '瓶', color: '#6aa0d0', price: 12, target: 'ally', desc: 'Might heal 25. Might be Angelo. Don\'t look too closely at the water.', field: true, use: x => { if (Math.random() < 0.65) x.heal(x.target, 25); else x.status(x.target, 'bleed', 4); } },
...
SBR.RELICS = {
  colt:       { name: 'Mista\'s Revolver', glyph: '銃', color: '#8a8a9a', rarity: 'starter', desc: 'Guns and nails deal +15% damage. Six chambers, never four bullets.', bonus: { gunDmg: 0.15 }, starter: true },
  ...
  provisions: { name: 'Speedwagon Foundation Crate', glyph: '糧', color: '#c8844a', rarity: 'starter', desc: 'Start with $60, an SPW Canteen, Kakyoin\'s Cherries and Iggy\'s Coffee Gum. +1 belt slot.', bonus: { belt: 1 }, starter: true, onGain: r => { r.money += 60; r.items.push('canteen', 'jerky', 'coffee'); } },
  ...
  spurs:      { name: 'Hol Horse\'s Spurs', glyph: '拍', color: '#f2c14e', rarity: 'common', price: 65, desc: '+5% dodge chance. Number Two is always first out of a losing fight.', bonus: { dodge: 0.05 } },
  cactusjuice:{ name: 'Hamon Breathing Mask', glyph: '波', color: '#c8c8d8', rarity: 'common', price: 75, desc: 'Party heals 1 HP each turn in battle. Lisa Lisa\'s training mask: breathe wrong and it cracks.', bonus: { regen: 1 } },
  bandolier:  { name: 'Bucciarati\'s Zipper Suit', glyph: '開', color: '#f6ecd8', rarity: 'common', price: 55, desc: '+2 belt slots for items. Sticky Fingers zipped extra pockets into it.', bonus: { belt: 2 } },
  ...
  scope:      { name: 'Boingo\'s Prophecy Comic', glyph: '予', color: '#f2c14e', rarity: 'common', price: 70, desc: '+5% crit chance, +15% crit damage. Thoth draws where the hit lands. Thoth is never wrong.', bonus: { crit: 0.05, critDmg: 0.15 } },
  ...
  rattle:     { name: 'Caesar\'s Bandana', glyph: '泡', color: '#3a8a6a', rarity: 'common', price: 65, desc: 'Basic attacks have 30% chance to inflict Bleed. Bubble Cutter: soap bubbles sharp enough to slice.', bonus: { bleedOnBasic: 0.3 } },
  hat:        { name: 'Speedwagon\'s Bowler Hat', glyph: '帽', color: '#3a3a4a', rarity: 'common', price: 50, desc: '+3% dodge, +3% crit. The brim is a razor.', bonus: { dodge: 0.03, crit: 0.03 } },
  codebook:   { name: 'Speedwagon\'s Oil Deed', glyph: '油', color: '#f0dcae', rarity: 'common', price: 55, desc: '+25% money from all sources. A share in a Texas oil field. It pays out every stage.', bonus: { money: 0.25 } },
  cowbell:    { name: 'Trattoria Trussardi Menu', glyph: '食', color: '#f6ecd8', rarity: 'common', price: 40, desc: 'Resting heals 20% more. Tonio\'s recipes: every course fixes something.', bonus: { restHeal: 0.2 } },
  // rare
  ...
  journal:    { name: 'DIO\'s Diary', glyph: '記', color: '#2a1a3a', rarity: 'rare', price: 110, desc: '+25% experience. Notes on how to reach Heaven.', bonus: { xp: 0.25 } },
